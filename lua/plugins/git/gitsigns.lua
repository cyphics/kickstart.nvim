-- Adds git-related signs to the gutter, plus keymaps for staging/navigating hunks.
...
return {
  {
    ...
    opts = {
      ...
      on_attach = function(bufnr)
        ...
        -- Actions (under <leader>gh — matches LazyVim's own "+hunks" group)
        -- visual mode
        ...
        -- Persistent toggles (capitalized to avoid clashing with the one-shot actions above)
        map("n", "<leader>ghT", gitsigns.toggle_current_line_blame, { desc = "git [T]oggle blame line" })
        map("n", "<leader>ghX", gitsigns.toggle_deleted, { desc = "git toggle deleted lines" })
      end,
    },
  },
}
